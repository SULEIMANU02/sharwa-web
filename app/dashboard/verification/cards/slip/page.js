...
export default function CardSlipPage() {
  const { register, handleSubmit, formState: { isSubmitting } } = useForm()
  const [message, setMessage] = useState('')
  const onSubmit = async (data) => {
    setMessage('')
    try {
      await api.post('/verify/card/slip', data)
      setMessage('NIN slip verification submitted')
    } catch (e) {
  ...
  return (
    <div className="max-w-md">
      <h2 className="text-xl font-semibold mb-4">NIN Slip Verification</h2>
