...
export default function CardStandardPage() {
  const { register, handleSubmit, formState: { isSubmitting } } = useForm()
  const [message, setMessage] = useState('')
  const onSubmit = async (data) => {
    setMessage('')
    try {
      await api.post('/verify/card/standard', data)
      setMessage('Standard card verification submitted')
    } catch (e) {
  ...
  return (
    <div className="max-w-md">
      <h2 className="text-xl font-semibold mb-4">Card Verification - Standard</h2>
