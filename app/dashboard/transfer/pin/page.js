...
export default function TransferPinPage() {
  ...
  const submit = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch('https://sharwadata.com.ng/purchase/transfer', {
        method: 'POST',
      ...
      if (result?.success) {
        router.push(`/dashboard/success?success=success&type=transfer&amount=${encodeURIComponent(payload.amount)}&time=${Date.now()}`)
        return
      }
      ...
      router.push(`/dashboard/success?success=failed&type=transfer&amount=${encodeURIComponent(payload.amount)}&time=${Date.now()}`)
    } catch (error) {
      alert('Unable to complete transfer. Please try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      ...
      <p className="text-sm text-gray-600 mb-5">Enter your 4-digit pin to authorize transfer.</p>
