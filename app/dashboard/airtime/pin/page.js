// app/dashboard/airtime/pin/page.js
'use client'
import { useState, useEffect } from 'react'
import { useRouter, useSearchParams } from 'next/navigation'

export default function AirtimePinPage() {
  ...
  const checkDb = async () => {
    ...
    try {
      const response = await fetch('https://sharwadata.com.ng/purchase/airtime', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        router.push(`/dashboard/success?network=${networks}&phone=${phoneNumber}&success=success&type=airtime&amount=${amount}&time=${new Date().getTime()}`)
      } else {
        if (result.error === 'invalid_token') {
          ...
          router.push(`/dashboard/success?network=${networks}&phone=${phoneNumber}&success=failed&type=airtime&amount=${amount}&time=${new Date().getTime()}`)
        }
      }
    } catch (error) {
  ...
  const getNetworkName = (networkCode) => {
    if (networkCode === '1') return 'MTN'
    if (networkCode === '4') return 'AIRTEL'
    if (networkCode === '2') return 'GLO'
    if (networkCode === '3') return '9MOBILE'
    return 'Unknown'
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
      ...
      <div className="text-center mb-12">
        ...
        {/* Transaction Details */}
        <div className="bg-white rounded-2xl p-6 shadow-lg max-w-sm mx-auto mb-8">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Airtime Purchase</h2>
          <div className="space-y-2 text-sm text-gray-600">
            <div className="flex justify-between">
              <span>Network:</span>
              <span className="font-medium">{getNetworkName(networks)}</span>
            </div>
            <div className="flex justify-between">
              <span>Amount:</span>
        ...
        <p className="text-gray-600 text-sm">Enter your 4-digit PIN to confirm transaction</p>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 shadow-2xl">
            <div className="flex flex-col items-center">
              ...
              <p className="text-gray-700 font-semibold">Processing airtime purchase...</p>
